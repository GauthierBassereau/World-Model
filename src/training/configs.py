...
from src.dataset.configs import DataloaderConfig, LeRobotDatasetConfig, WorldModelDatasetConfig
...
class TrainDataConfig:
    train_dataset: WorldModelDatasetConfig = field(default_factory=lambda: WorldModelDatasetConfig(datasets={}, weights={}))
    train_dataloader: DataloaderConfig = field(default_factory=DataloaderConfig)


@dataclass
class EvalDataConfig:
    eval_dataset: LeRobotDatasetConfig = field(
        default_factory=lambda: LeRobotDatasetConfig(episode_midpoint_only=True)
    )
